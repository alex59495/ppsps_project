class Api::V1::PpspsController < Api::V1::BaseController
  ...
  def index
    ...
    @ppsps = params[:page] ? policy_scope(Ppsp.includes(:user, :project_information, :worksite).where(user: users).page(params[:page]).per(12).order(updated_at: :desc)) : policy_scope(Ppsp.includes(:user, :project_information, :worksite).where(user: users).order(updated_at: :desc))

    # If params search exist then will filter with reference and/or user first and last names
    @ppsps = params[:search].present? ? @ppsps.where("CONCAT_WS(' ', users.first_name, users.last_name) ILIKE :query or project_informations.reference ILIKE :query", query: "%#{params[:search]}%").references(:users, :project_informations) : @ppsps
  end
