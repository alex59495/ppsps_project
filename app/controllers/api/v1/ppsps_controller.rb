class Api::V1::PpspsController < Api::V1::BaseController
  ...
  def index
    ...
    params[:search].present? ? @ppsps = @ppsps.includes(:user, :project_information).where("CONCAT_WS(' ', users.first_name, users.last_name) ILIKE ? or project_informations.reference ILIKE ?", "%#{params[:search]}%", "%#{params[:search]}%").references(:users, :project_informations) : @ppsps
  end
